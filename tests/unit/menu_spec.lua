...
describe("menu:", function()
  ...
  before_each(function()
    ...
    stub(history_mock, "get_type_by_component").returns(T_Type.METHOD)
  ...
  it("should mount the popup menu when called", function()
    ...
    assert.stub(vim.api.nvim_buf_set_option).was_called(3)
  end)
  ...
  it("should run correct command when item is selected", function()
    ...
    assert
      .stub(terminal_mock.run_command_in_terminal)
      .was_called_with("mvn test -Dtest=TestClass#TestMethod", TEST_METHOD, T_Type.METHOD)
  end)
  ...
  it("should show correct info when item is selected", function()
    ...
    assert.stub(util_mock.notify_tests_running).was_called_with(TEST_METHOD, T_Type.METHOD)
  end)
end)
